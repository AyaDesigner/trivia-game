...
const Game = () => {
    ...
    const [fetchQuestions, isLoading, error] = useLoadingCallback(
        async () => {
            ...
            allTheAnswers.push({
                text: questions.results[questionIndex].correct_answer.replace(/[!@#$^&%*()+=[\]/{}|:<>?,.\\-]/g, ''),
                correct: true
            });
            ...
            <Grid item xs={6}>
                <Box m={2} pt={10}>
                    ...
                    <Card className={classes.root} m={3}>
                        <CardContent>
                            {showPoints ?
                                <Typography>You scored {points} out of {allQuestions.length}</Typography> :
